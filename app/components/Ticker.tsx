const Ticker = ({ titles }: { titles: string[] }) => {
  ...
  const tickerKey = base.join("-");

  return (
    <div className="overflow-hidden border-t-2 border-t-neon-pink border-b border-neon-pink/25 bg-neon-pink/2 py-2 sm:py-2">
      <div
        key={tickerKey}
        className="flex w-max animate-[ticker_40s_linear_infinite] whitespace-nowrap"
      >
        {repeated.map((t, i) => (
          <span key={i} className="vhs-badge mx-6 text-[11px]! sm:text-[12px]! text-neon-pink/60">
            ▶ {t}
          </span>
        ))}
      </div>
